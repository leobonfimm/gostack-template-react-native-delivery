...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      const responseExtras = responseFood.data.extras.map(item => {
        return {
          ...item,
          quantity: 0,
        };
      ...
      const favoritesFood = favorites.data.filter(
        item => item.id === responseFood.data.id,
      );

      setIsFavorite(favoritesFood.length > 0);
    }
